        ...
        if 'properties' in schema:
            for name, prop in schema['properties'].items():
                ...
                if isclass(field.annotation) and issubclass(field.annotation, Enum):
                    ...
                    prop['type'] = get_type_str(type(next(iter(field.annotation)).value))
                ...
                # Handle dictionaries
                    ...
                    if isclass(value_type) and issubclass(value_type, BaseModel):
                        prop['additionalProperties'] = refine_schema(value_type.schema(), value_type)
                    else:
                ...
                # Handle nested Pydantic models
                    ...
                    prop.update(refine_schema(field.annotation.schema(), field.annotation))

        schema['title'] = model.__name__
        schema['description'] = model.__doc__.strip() if model.__doc__ else ""
        schema['required'] = [name for name, field in model.__fields__.items() if field.is_required]
        if "$defs" in schema:
            schema.pop("$defs")
        return schema
