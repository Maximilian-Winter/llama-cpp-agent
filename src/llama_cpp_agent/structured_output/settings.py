...
class LlmOutputType(Enum):
    """
    Enum for defining different types of outputs that can be generated by a Language Model.
    """
...
class StructuredOutputSettings(BaseModel):
    """
    Settings for structured output generation from Language Models using tools like function calling and pydantic model generation.

    Attributes:
        output_type (Optional[LlmOutputType]): Defines the type of structured output.
    ...
    output_type: Optional[LlmOutputType] = Field(
        LlmOutputType.no_structured_output, description="The output type of the llm"
    )
    ...
    def from_llama_cpp_function_tools(
        llama_cpp_function_tools: List[LlamaCppFunctionTool], output_type: LlmOutputType
    ):
        """
        ...
        Args:
            ...
            output_type (LlmOutputType): Desired output type. Has to be either LlmOutputType.function_call or LlmOutputType.parallel_function_call.

        Returns:
            StructuredOutputSettings: Configured settings object.
        """
    ...
    def from_pydantic_models(models: List[BaseModel], output_type: LlmOutputType):
        ...
        Args:
            ...
            output_type (LlmOutputType): Desired output type.
        ...
        if output_type is LlmOutputType.no_structured_output:
            ...
            return StructuredOutputSettings(
                output_type=LlmOutputType.object_instance, pydantic_models=models
            )
        elif output_type is LlmOutputType.list_of_objects:
            return StructuredOutputSettings(
                output_type=LlmOutputType.list_of_objects, pydantic_models=models
            )
        elif output_type is LlmOutputType.function_calling:
            return StructuredOutputSettings(
                output_type=LlmOutputType.function_calling,
                function_tools=[LlamaCppFunctionTool(model) for model in models],
            )
        elif output_type is LlmOutputType.parallel_function_calling:
            return StructuredOutputSettings(
                output_type=LlmOutputType.parallel_function_calling,
                function_tools=[LlamaCppFunctionTool(model) for model in models],
            )

    @staticmethod
    def from_open_ai_tools(
        tools: List[Tuple[Dict[str, Any], Callable]], output_type: LlmOutputType
    ):
        """
    ...
    def add_pydantic_model(self, model: BaseModel):
        ...
        if self.output_type is LlmOutputType.no_structured_output:
            ...
            self.pydantic_models.append(model)
        elif self.output_type is LlmOutputType.function_calling:
            self.function_tools.append(LlamaCppFunctionTool(model))
        elif self.output_type is LlmOutputType.parallel_function_calling:
            self.function_tools.append(LlamaCppFunctionTool(model))
    ...
    def get_llm_documentation(self):
        ...
        if self.output_type == LlmOutputType.no_structured_output:
            ...
            return generate_text_documentation(self.pydantic_models)
        elif self.output_type == LlmOutputType.function_calling:
            return generate_text_documentation(
                [tool.model for tool in self.function_tools],
                model_prefix="Function",
                fields_prefix="Parameters",
            )
        elif self.output_type == LlmOutputType.parallel_function_calling:
            return generate_text_documentation(
                [tool.model for tool in self.function_tools],
                model_prefix="Function",
                fields_prefix="Parameters",
