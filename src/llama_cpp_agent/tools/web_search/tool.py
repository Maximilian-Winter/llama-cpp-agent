...
from ...prompt_templates import summarizing_system_prompt, general_summarizing_system_prompt, \
        ...
        for res in results:
            ...
            if web_info != "":
                ...
                web_info = self.summarising_agent.get_chat_response(
                    web_info, system_prompt=PromptTemplate.from_string(summarizing_system_prompt).generate_prompt({"QUERY": search_query, "WEBSITE_URL": res}),
...
class WebSummarizerTool:

    def __init__(self, llm_provider: LlmProvider, message_formatter_type: MessagesFormatterType,
                 web_crawler: WebCrawler = None, temperature: int = 0.45,
                 top_p: int = 0.95,
                 top_k: int = 40,
                 model_max_context_tokens=8192,
                 max_tokens_summary_results: int = 7500,
                 max_tokens_per_summary: int = 750):
        self.llm_provider = llm_provider
        self.summarising_agent = LlamaCppAgent(llm_provider, debug_output=True,
        ...
        self.max_tokens_search_results = max_tokens_summary_results
        settings = llm_provider.get_provider_default_settings()
        provider_id = llm_provider.get_provider_identifier()
        settings.temperature = temperature
        settings.top_p = top_p
    ...
    def summarize_urls(self, urls: List[str]):
        """
        Gets the content of a list of URLs and summarizes the content. Returns the results as a string.
        Args:
            urls (str): A list of URLs to summarize.
        """
        result_string = ''
        for res in urls:
            web_info = self.web_crawler.get_website_content_from_url(res)
            if web_info != "":
                ...
                if has_remove_char:
                    web_info = web_info[:remove_char_count]
                web_info = self.summarising_agent.get_chat_response(
                    f"Please summarize the following Website content and extract all information.\n\n" + web_info,
                    add_response_to_chat_history=False, add_message_to_chat_history=False,
                    llm_sampling_settings=self.settings)
                result_string += f"\n{web_info.strip()}"
...
class SummarizerTool:
    ...
    def summarize_text(self, query, input_text: List[str]):
        """
        Summarizes the list of input texts.
        Args:
            input_text (str): A list of texts to summarize.
        """
        result_strings = []
        for res in input_text:
            web_info = res
            if web_info != "":
                tokens = self.llm_provider.tokenize(web_info)
                original_prompt_token_count = len(tokens)
                ...
                if has_remove_char:
                    web_info = web_info[:remove_char_count]
                web_info = self.summarising_agent.get_chat_response(
                    web_info, system_prompt=PromptTemplate.from_string(summarizing_system_prompt_ocr).generate_prompt({"QUERY": query}),
                    add_response_to_chat_history=False, add_message_to_chat_history=False,
                    llm_sampling_settings=self.settings)
...
class PdfSummarizerTool:
    def __init__(self, summarizer_tool: SummarizerTool):
        self.summarizer_tool = summarizer_tool

    def summarize_pdfs(self, query, texts: List[str]):
        return self.summarizer_tool.summarize_text(query, texts)
